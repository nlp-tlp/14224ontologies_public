...
def transform_local(name: str) -> str:
    # Rule 1: underscore terms -> UpperCamelCase
    if "_" in name:
        return to_upper_camel_from_underscore(name)
    # Rule 2: single-word all-lower -> Capitalize first letter
    if is_single_word_all_lower(name):
        return name[:1].upper() + name[1:]
    # Otherwise: leave unchanged (e.g., TypeTest stays TypeTest)
    return name
...
def carry_over_pref_from_rdfs(g_in: Graph, g_out: Graph, src: URIRef, tgt: URIRef) -> None:
...
def main() -> None:
    ...
    # ConceptScheme (THIS LINE MUST NOT WRAP MID-STRING)
    ...
    class_to_source_uri: dict[URIRef, URIRef] = {}
    ...
    # ---------------------------------------
    # Named Individuals -> skos:narrower links
    # ---------------------------------------
    ...
    for ind in sorted(individuals, key=lambda x: str(x)):
        ...
        if linked_any:
            ...
            if not any(g_out.objects(ind_concept, SKOS.prefLabel)):
                g_out.add((ind_concept, SKOS.prefLabel, Literal(new_ln, lang="en")))
